...
begin #move function
    function move(path::Path, bead::Int, h::Float64, potential::Potential)
        ...
        if bead < path.n_beads
            old_action =
                action(path, path.positions[bead], path.positions[bead+1], potential)
...
begin #energy
    ...
    function Kinetic_energy(path, estimator::ThermodynamicEstimator)
        ...
        for i = 1:path.n_beads
            ...
            elseif i == 1
                link_term =
                    ((path.positions[i] - path.positions[path.n_beads])^2) * link_factor
            end
    ...
    function Energy(path::Path, estimator::Estimator, potential::Potential)
        return (Kinetic_energy(path, estimator) + Potential_energy(path, potential)) /
               path.n_beads
    end
end

begin #ground state wave function
    function wave_function(path::Path) end
end
