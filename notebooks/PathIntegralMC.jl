...
begin
    ...
    function one_body_potential(
        ...
        ω = potential.ω
        phonon_response(other_bead) =
            cosh(ω * path.τ * (abs(bead - other_bead) - path.n_beads / 2.0)) /
            sinh(ω * path.τ * path.n_beads / 2.0)
        double_integral = 0.0
        for other_bead = 1:path.n_beads
            if other_bead != bead
                double_integral +=
                    phonon_response(other_bead) / norm(
                        path.beads[bead, particle, :] .-
                        path.beads[other_bead, particle, :],
                    )
            end
        end
    ...
    function two_body_potential(
        ...
        return -potential.κ /
               norm(path.beads[bead, particle_one, :] .- path.beads[bead, particle_two, :])
    end
end
...
begin #action
    ...
    function kinetic_action(path::Path, bead::Int, particle::Int)
        ...
        kinetic_action = (
            norm(path.beads[bead, particle, :] - path.beads[bead_before, particle, :])^2 +
            norm(path.beads[bead, particle, :] - path.beads[bead_after, particle, :])^2
        )
    ...
    function potential_action(
        ...
        for particle_other = 1:path.n_particles
            if particle_other != particle
                potential_action +=
                    two_body_potential(potential, path, bead, particle, particle_other)
            end
        end
...
begin #Combined action
    function primitive_action(path::Path, bead::Int, particle::Int, potential::Potential)
        return kinetic_action(path, bead, particle) +
               potential_action(path, bead, particle, potential)
    end

    function primitive_action(
        path::Path,
        bead::Int,
        particle::Int,
        potentials::Array{Potential},
    ...
    function kinetic_energy(path::Path)
        ...
        for bead_one = 1:path.n_beads
            ...
            for particle = 1:path.n_particles
                kinetic_energy -=
                    norm(
                        path.beads[bead_one, particle, :] -
                        path.beads[bead_two, particle, :],
                    )^2
            end
        end
        return prefactor * kinetic_energy / path.n_beads +
               path.n_dimensions * path.n_particles / (2 * path.τ)
    end
...
begin #PIMC
    function PIMC(
        ...
        observables,
        path::Path,
        potentials::Union{Potential,Array{Potential}},
    )
        ...
        observable_traces =
            Dict(string(Symbol(observable)) => [] for observable in observables)
        ...
        for step = 1:n_steps
            ...
            if mod(step, observable_skip) == 0 && step > equilibrium_skip
                for observable in observables
                    append!(
                        observable_traces[string(Symbol(observable))],
                        [observable(path, potentials)],
                    )
                end
        ...
        acceptance_ratio = Dict(
            string(Symbol(mover)) =>
                1.0 * n_accepted[string(Symbol(mover))] / (n_steps * path.n_particles)
            for mover in movers
        )
...
begin
    function Correlation(path::Path, potential::Potential)
        ...
        for Δτ = 1:path.n_beads, bead_one = 1:path.n_beads
...
begin
    ...
    function Staging(
        ...
        old_beads = copy(path.beads[:, particle, :])
        old_action = 0.0
        for bead = 2:segment_length
            old_action += potential_action(path, bead, particle, potentials)
        end

        new_action = 0.0
        for bead = 1:segment_length-1
            staging_mass = (segment_length - bead + 1) / (segment_length - bead)
            staging_position =
                (
                    path.beads[1+segment_length, particle, :] +
                    path.beads[bead, particle, :] * (segment_length - bead)
                ) / (segment_length - bead + 1)
            path.beads[bead+1, particle, :] =
                staging_position + randn(path.n_dimensions) * sqrt(path.τ / staging_mass)
