...
DiagMC_α_E = [-1.08, -2.13, -3.2, -4.36, -5.59, -6.91, -8.42, 
                -9.98, -11.77, -13.8, -16.05, -18.52]

Var_data = []
variational_run = feynmanvw.(2.0, 1.0, collect(α_range), 1.0, Inf)
for i in 1:length(variational_run)
    push!(Var_data, variational_run[i][3])
end

Frohlich_plot = plot(α_range, WeakPTFrohlich.(α_range), labels = "WeakPT", linestyle=:dash, 
                    ylabel = L"Energy \ E(α) \ (T=0.1)", xlabel = L"α", dpi=300)
plot!(α_range, strongPTFrohlich.(α_range), labels = "StrongPT", linestyle=:dash, dpi=300)
