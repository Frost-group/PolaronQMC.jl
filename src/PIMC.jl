...
function PIMC(
    ...
    estimators_string =
        [split(string(Symbol(estimator)), "Estimator()")[1] for estimator in estimators]
    ...
    if typeof(potential) == FrohlichPotential
        for k = 1:length(potential.ω)
            fac = potential.ħ * path.τ * path.n_beads * potential.ω[k]
            for i = 1:path.n_beads, j = i+1:path.n_beads
        ...
        for particle in rand(1:path.n_particles, path.n_particles)
            ...
            for sweep = 1:n_sweep
                moveBead!(
                    ...
                    store_diff,
                    coshfn,
                ) # Moving beads a total of n_sweep times
            end
            ...
            if adjust
                ...
                data["Acceptance Rate:p$(particle)"][step] =
                    mover.adjusters[particle].acceptance_rate
        ...
        if mod(step, observable_skip) == 0 && step > equilibrium_skip
            ...
            if "Energy" in observables_set
                for (index, estimator) in enumerate(estimators_string)
                    push!(
                        data["Energy:$(estimator)"],
                        energy(path, potential, estimators[index], store_diff, coshfn),
                    )
                end
            end
            ...
            if "Position" in observables_set
                for particle = 1:path.n_particles
                    for dimension = 1:path.n_dimensions
                        push!(
                            data["Position:p$(particle)d$(dimension)"],
                            collect(Iterators.flatten(path.beads[:, particle, dimension])),
                        )
                    end
                end
            end
            ...
            if "Correlation" in observables_set
                for (estimator, estimator_string) in zip(estimators, estimators_string)
                    push!(
                        data["Correlation:$(estimator_string)"],
                        correlation(path, potential, estimator),
                    )
                end
            end
        end
...
function Holstein_PIMC(
    ...
    for l = 1:path.n_beads
        F_l[l] =
            2 * path.n_dimensions * path.τ^3 * potential.α / (4 * path.n_beads) * sum(
                (
                    cos(2π * (k - 1) * (l - 1) / path.n_beads) /
                    (1 - cos(2π * (k - 1) / path.n_beads) + path.τ^2 * potential.ω^2 / 2)
                ) for k = 1:path.n_beads
            )
    end
    ...
    for l = 1:path.n_beads
        DF_l[l] =
            2 * path.n_dimensions * path.τ^2 * potential.α / (4 * path.n_beads) * (
                3 * sum(
                    (
                        cos(2π * (k - 1) * (l - 1) / path.n_beads) / (
                            1 - cos(2π * (k - 1) / path.n_beads) +
                            path.τ^2 * potential.ω^2 / 2
                        )
                    ) for k = 1:path.n_beads
                ) -
                path.τ^2 *
        ...
        if mod(step, observable_skip) == 0 && step > equilibrium_skip
            ...
            if "Energy" in observables_set
                for (index, estimator) in enumerate(estimators_string)
                    push!(
                        data["Energy:$(estimator)"],
                        energy(path, potential, estimators[index], DF_l),
                    )
                end
            end
