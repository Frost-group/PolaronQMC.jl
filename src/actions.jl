...
function totalAction(
    ...
    return kinecticAction(path, bead_one, bead_two, particle, regime) +
           potential_action(path, bead_one, particle, potential, regime)
end
...
function kineticAction(
    ...
    """
    kinetic_action::Float64 =
        0.5 *
        path.m[particle] *
        norm(
            path.beads[mod1(bead_two, path.n_beads), particle, :] -
            path.beads[mod1(bead_one, path.n_beads), particle, :],
        )^2 / path.τ
...
function kineticAction(
    ...
    for i = 1:path.n_dimensions
        store_diff[i] =
            path.beads[mod1(bead_two, path.n_beads), particle, i] -
            path.beads[mod1(bead_one, path.n_beads), particle, i]
    end
...
function kineticAction(
    ...
    for dimension = 1:path.n_dimensions
        kinetic_action *= besseli(
            path.beads[bead_one, particle, dimension] -
            path.beads[bead_two, particle, dimension],
...
function potentialAction(
    ...
    potential_action = sum(
        twoBodyPotential(potential, path, bead, particle, other_particle) for
        other_particle = 1:path.n_particles if particle != other_particle
    )
...
function potentialAction(
    ...
    V2_prefactor =
        (
            0.5 *
            potential.α *
            (potential.ħ * potential.ω)^(3 / 2) *
            sqrt(1 / 2 / path.m) *
            csch(potential.ħ * potential.ω * β / 2)
        )^2
    ħω = potential.ω * potential.ħ
    # F = -dV/dr ∝ r/(r(τ)-r(τ'))^3

    inner_integral = 0.0
    for other_bead = 1:path.n_beads+1
    ...
    return 2 * path.τ * oneBodyPotential(potential, path, bead, particle) +
           2 * inner_integral * path.τ^2 * t2_prefactor * V2_prefactor
end

function potentialAction(
    path::Path,
    bead::Int,
    beadrange::Union{Vector{Int64},StepRange{Int64,Int64},UnitRange{Int64}},
    particle::Int,
    potential::FrohlichPotential,
    regime::PrimitiveRegime,
)
    ...
    for other_bead = 1:n_beads
        if other_bead != bead
            if other_bead in beadrange
                inner_integral +=
                    0.5 * cosh(ħω * β * (abs(bead - other_bead) / n_beads - 0.5)) / norm(
                        path.beads[mod1(bead, n_beads), particle, :] -
                        path.beads[mod1(other_bead, n_beads), particle, :],
                    )
            else
                inner_integral +=
                    cosh(ħω * β * (abs(bead - other_bead) / n_beads - 0.5)) / norm(
                        path.beads[mod1(bead, n_beads), particle, :] -
                        path.beads[mod1(other_bead, n_beads), particle, :],
                    )
            end
