...
function kineticEnergy(
    path::Path,
    potential::Potential,
    estimator::Union{SimpleEstimator,SimpleVirialEstimator},
) #thermal dynamic estimator from ceperly paper
    kinetic_energy = 0.0

    # Add-up energy for each particle and each bead
    for bead = 1:path.n_beads, particle = 1:path.n_particles
        kinetic_energy +=
            0.5 *
            path.m[particle] *
            norm(
                path.beads[bead, particle] -
                path.beads[mod1(bead - 1, path.n_beads), particle],
            )^2 / path.τ^2
    end
    ...
    for bead = 1:path.n_beads, particle = 1:path.n_particles
        kinetic_energy +=
            norm(
                ...
                @view(path.beads[mod1(bead - 1, path.n_beads), particle, :]),
            )^2
    end
...
function kineticEnergy(
    path::Path,
    potential::Union{HarmonicPotential,HarmonicInteractionPotential},
    estimator::VirialEstimator,
    store_diff::Vector{Float64},
)
    """
    ...
    for particle = 1:path.n_particles
        centroid_pos = [
            ThreadsX.sum(
                path.beads[mod1(bead, path.n_beads), particle, dimension] for
                bead = 1:path.n_beads
            ) for dimension = 1:path.n_dimensions
        ]
        centroid_pos /= path.n_beads

        term_two += ThreadsX.sum(
            dot(
                path.beads[mod1(bead, path.n_beads), particle, :] - centroid_pos,
...
function kineticEnergy(
    ...
    function getTermTwo(particle, bead, other_bead, centroid_pos, ω)
        if bead != other_bead
            ...
            #=
            g_factor = (path.τ^2 * sinh(potential.ħ * β * ω * (abs(other_bead-bead)/n_beads-0.5)) * potential.ħ * ω * n_beads * (abs(other_bead-bead)/n_beads-0.5)
            - path.τ^2 * cosh(potential.ħ * ω * β / 2) /sinh(potential.ħ * ω * β / 2) * cosh(potential.ħ * ω * β * (abs(bead-other_bead)/n_beads - 0.5)) * 0.5 * potential.ħ * ω * n_beads
            + 2 * path.τ * cosh(potential.ħ * ω * β * (abs(bead-other_bead)/n_beads - 0.5)))
            =#
            for i = 1:path.n_dimensions
                store_diff[i] =
                    path.beads[bead, particle, i] - path.beads[other_bead, particle, i]
            end
            ...
            return g_factor *
                   dot((@view(path.beads[bead, particle, :]) - centroid_pos), store_diff) /
                   norm(store_diff)^3
        else
            return 0.0
        end
    ...
    for ω in potential.ω
        ...
        t2_prefactor =
            ...
            (potential.ħ * ω)^(3 / 2) *
            sqrt(1 / 2 / path.m) *
            csch(potential.ħ * ω * β / 2)
        #t2_prefactor = 1/(2 * (n_beads)) * 0.5 * potential.α * (potential.ħ * ω)^(3/2) * sqrt(1/2/path.m) * csch(potential.ħ * ω * β / 2)
        ...
        for particle = 1:path.n_particles
            centroid_pos =
                [
                    sum(path.beads[bead, particle, dimension] for bead = 1:n_beads) for
                    dimension = 1:path.n_dimensions
                ] / n_beads
            term_two += sum(
                getTermTwo(particle, bead, other_bead, centroid_pos, ω) for
                bead = 1:n_beads, other_bead = 1:n_beads
            )
        end
...
function kineticEnergy(
    ...
    estimator::PIMCEstimator,
)
    kinetic_energy = 0.0
    for particle = 1:path.n_particles
        for i = 1:path.n_beads
            for k = 1:path.n_dimensions
                kinetic_energy +=
                    (
                        besseli(
                            path.beads[i, particle, k] -
                            path.beads[mod1(i + 1, path.n_beads), particle, k] - 1,
                            2 * path.τ * potential.J,
                        ) + besseli(
                            path.beads[i, particle, k] -
                            path.beads[mod1(i + 1, path.n_beads), particle, k] + 1,
                            2 * path.τ * potential.J,
                        ...
                        path.beads[i, particle, k] -
                        path.beads[mod1(i + 1, path.n_beads), particle, k],
                        2 * path.τ * potential.J,
                    ))
...
function potentialEnergy(
    ...
    # Calculates the double integral component with individual mode
    for particle = 1:path.n_particles
        for bead = 1:n_beads
            for other_bead = 1:n_beads
                if other_bead != bead # Discounting self-contributions
                    for i = 1:path.n_dimensions
                        store_diff[i] =
                            path.beads[mod1(bead, n_beads), particle, i] -
                    ...
                    for i = 1:l_ω
                        ...
                        potential_energy +=
                            ...
                            sqrt(1 / 2 / path.m) / norm(store_diff) *
                            ...
                            (
                                ...
                                sinh(
                                    ...
                                    β *
                                    potential.ω[i] *
                                    (abs(other_bead - bead) / path.n_beads - 0.5),
                                ) *
                                potential.ħ *
                                potential.ω[i] *
                                path.n_beads *
                                (abs(other_bead - bead) / path.n_beads - 0.5) -
                                path.τ^2 * cosh(potential.ħ * potential.ω[i] * β / 2) /
                                sinh(potential.ħ * potential.ω[i] * β / 2) *
                                prop_Matrix[
                                    max(bead, other_bead),
                                    min(bead, other_bead),
                                    i,
                                ...
                                2 *
                                path.τ *
                                prop_Matrix[max(bead, other_bead), min(bead, other_bead), i]
                            )
                        #- path.τ * prop_Matrix[max(bead, other_bead), min(bead, other_bead), i] * csch(potential.ħ * potential.ω[i] * β / 2)/norm(store_diff)^2)
...
function potentialEnergy(path::Path, potential::FrohlichPotential, estimator::PIMCEstimator, store_diff::Vector{Float64}, prop_Matrix::Array{Float64})
    ...
    potential_energy = 0.0 # The final after summing all beads
    for particle in path.n_particles
        for bead in 1:n_beads
            for other_bead in bead+1:n_beads
                ...
                #if A > 0.01
                for i in 1:l
                    potential_energy += -0.5 * α * (potential.ħ * potential.ω[i])^(3/2) * sqrt(1/2/path.m) * csch(potential.ħ * potential.ω[i] * β / 2) * 
                                    prop_Matrix[other_bead, bead, i]/norm(store_diff)
                end
                #end
            end
        end
...
function potentialEnergy(
    path::Path,
    potential::OneBodyPotential,
    estimator::PIMCEstimator,
    store_diff::Vector{Float64},
    prop_Matrix::Array{Float64},
)
    return sum(
        oneBodyPotential(potential, path, bead, particle, store_diff, prop_Matrix) /
        (path.n_beads) for bead = 1:path.n_beads, particle = 1:path.n_particles
    )
end
...
function potentialEnergy(
    ...
    potential_energy = 0.0
    for bead = 1:path.n_beads,
        particle_one = 1:path.n_particles,
        particle_two = 1:path.n_particles

        if particle_one != particle_two
            potential_energy +=
                twoBodyPotential(potential, path, bead, particle_one, particle_two)
        end
    end
    return potential_energy / path.n_beads
end
...
function correlation(path::Path, potential::HarmonicPotential, estimator::PIMCEstimator)
    ...
    for Δτ = 1:(path.n_beads-1)
        for bead_one = 1:path.n_beads
